	...
	updateSessionStatus(isSpeaking) {
		...
		if (isSpeaking === null) {
			...
			sessionStatus.innerHTML =
				bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> 小麦离线中</span>'
			sessionStatus.className = 'status offline'
		} else if (isSpeaking) {
			// 说话中
			sessionStatus.innerHTML =
				bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> 小麦说话中</span>'
			sessionStatus.className = 'status speaking'
		} else {
			// 聆听中
			sessionStatus.innerHTML =
				bgHtml + '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> 小麦聆听中</span>'
