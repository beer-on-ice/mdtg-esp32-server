											...
											<div style="display: flex; align-items: center; justify-content: space-between">
												<span style="color: #606266; font-size: 13px">
													{{ $t('roleConfig.contextProviderSuccess', { count: currentContextProviders.length })
														...
														href="https://github.com/beer-on-ice/mdtg-esp32-server/blob/main/docs/context-provider-integration.md"
														target="_blank"
														class="doc-link"
														>{{ $t('roleConfig.contextProviderDocLink') }}</a
													>
												</span>
									...
									<div class="form-column">
										...
										>
											<div class="model-select-wrapper">
												...
												<div
													v-if="model.type === 'Memory' && form.model.memModelId !== 'Memory_nomem'"
													...
													<div style="display: flex; justify-content: space-between; align-items: center">
														...
														<template v-if="hasAudioPreview(item)">
															<el-button
																type="text"
																:icon="
																	playingVoice && currentPlayingVoiceId === item.value && !isPaused
																		? 'el-icon-video-pause'
																		: 'el-icon-video-play'
																"
																size="small"
...
export default {
	...
	methods: {
		...
		fetchAgentConfig(agentId) {
			Api.agent.getDeviceConfig(agentId, ({ data }) => {
				if (data.code === 0) {
					...
					const ensureFuncs = this.allFunctions.length ? Promise.resolve() : this.fetchAllFunctions()
		...
		updateIntentOptionsVisibility() {
			...
			if (this.form.model.intentModelId === 'Intent_function_call' && llmType !== 'openai' && llmType !== 'ollama') {
				// 找到第一个可见的选项
		...
		hasAudioPreview(item) {
			...
			const isCloneAudio = item.hasCloneAudio || (item.value && !item.value.startsWith('TTS'))
		...
		playVoicePreview(voiceId = null) {
			...
			try {
				...
				if (voiceDetail) {
					...
					isCloneAudio = voiceDetail.hasCloneAudio || (voiceDetail.id && !voiceDetail.id.startsWith('TTS'))
