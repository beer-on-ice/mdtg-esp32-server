					...
					<div v-if="unselected.length">
						<div v-for="func in unselected" :key="func.name" class="function-item">
							<el-checkbox
								...
								@change="(val) => handleCheckboxChange(func, val)"
								@click.native.stop
							></el-checkbox>
				...
				<div v-if="currentFunction" class="params-container">
					<el-form :model="currentFunction" class="param-form">
						...
						<el-form-item
							...
							:label="field.label"
							class="param-item"
							:class="{ 'textarea-field': field.type === 'array' || field.type === 'json' }"
						>
							<template #label>
				...
				<div class="mcp-left">
					...
					<div class="url-header">
						<div class="address-desc">
							...
							<a
								href="https://github.com/beer-on-ice/mdtg-esp32-server/blob/main/docs/mcp-endpoint-enable.md"
								target="_blank"
								class="doc-link"
								>{{ $t('functionDialog.howToDeployMcp') }}</a
							>
							&nbsp;&nbsp;|&nbsp;&nbsp;
							<a
								href="https://github.com/beer-on-ice/mdtg-esp32-server/blob/main/docs/mcp-endpoint-integration.md"
								target="_blank"
								class="doc-link"
								>{{ $t('functionDialog.howToIntegrateMcp') }}</a
							>
							&nbsp;
						</div>
					</div>
				...
				<div class="mcp-right">
					...
					<div class="status-container">
						...
						<span class="status-text">{{
							mcpStatus === 'connected'
								? $t('functionDialog.connected')
								: mcpStatus === 'loading'
								? $t('functionDialog.loading')
								: $t('functionDialog.disconnected')
						}}</span>
		...
		saveSelection() {
			...
			const selected = this.selectedList.map((f) => {
				...
				return {
					id: f.id,
					name: f.name,
					params: modified ? { ...modified.params } : { ...f.params }
				}
			})
